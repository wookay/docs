...

{
    ...
    "category": "page",
    "text": "🦉  https://github.com/MikeInnes/DataFlow.jl/blob/master/docs/vertices.md 번역DataFlow가 하는 두가지:그래프 데이터 구조(a graph data structure)\n그래프를 기술하기 위한 공통 문법(a common syntax for describing graphs)서로 얶매여 있지 않으니 편하게 쓰면 된다;  예를 들어, 이 문법으로 만든 그래프를 인접 행렬(an adjacency matrix)로 변환하여 처리한다거나 DataFlow의 공통 그래프 연산 라이브러리를 활용하여 다른 방식으로 그래프를 생성할 수 있다.DataFlow는 명시적으로 데이터 구조를 단순하게 유지하고, 다른 어떠한 의미도 덧붙이지 않는다. 그래프는 반듯한 줄리아 프로그램, 베이시안 네트워크, 또는 전기 회로와 같은 것을 표현할 수 있다. (Flux와 같은) DataFlow를 사용하는 라이브러리는 문법을 확장하고 응용 프로그램에 적절한 코드를 생성하기 위해 그래프를 다룰 것이다."
},
